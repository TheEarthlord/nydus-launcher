...
def get_server_log_path():
    return os.path.join(get_log_dir(), "nydus-server.log") 

"""
This function empties the Nydus server logfile.
It should be called when a new instance of the Nydus server
starts, so the logfile doesn't grow forever.
Returns nothing
"""
def restart_server_log():
    with open(get_server_log_path(), "w") as f:
        f.flush()
...
into the nydus launcher server logs
...
in the nydus server's logfile
"""
def log_server(message):
    message = prepare_log_message(message)
    with open(get_server_log_path(), "a") as f:
        f.write(message)
        f.flush()
